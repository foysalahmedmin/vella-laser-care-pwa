...
const AddCart = ({ product }: { product: ProductDetails }) => {
  ...
  return (
    <div className="container">
      <div className="bg-card border-t p-4 py-6">
        <div className="flex items-center justify-between">
          ...
          <Button
            ...
            className="primary flex w-3/5 items-center justify-center rounded-xl p-3"
          >
            <span className="mr-2 font-bold text-white">
              {language.code === "en" ? "ADD TO CART" : "কার্টে যুক্ত করুন"}
            </span>
            <ShoppingCart strokeWidth={3} size={20} color="white" />
          </Button>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="container">
      <div className="bg-card p-6">
        ...
        <div className="flex items-center space-x-2">
          <span className="bg-primary/10 text-primary flex items-center rounded px-3 py-1 text-sm">
            {language.code === "en"
      ...
      <div className="bg-card shadow-sm">
        <Tabs defaultValue="highlight">
          <Tabs.List className="border-b">
            <Tabs.Trigger className="py-4" value="highlight">
              {language.code === "en" ? "Highlight" : "হাইলাইট"}
            </Tabs.Trigger>
            <Tabs.Trigger className="py-4" value="benefits">
              {language.code === "en" ? "Key Benefits" : "মূল সুবিধা"}
            </Tabs.Trigger>
            <Tabs.Trigger className="py-4" value="ingredients">
...
// Shipping Component
const Shipping = ({ product }: { product: ProductDetails }) => {
  const { language } = useLanguage();

  return (
    <div className="container">
      <div className="bg-card border-t p-6 shadow-sm">
        ...
        <button className="text-primary my-4 underline">
          {language.code === "en"
...
const ViewProduct = ({ media }: { media: ProductDetails["media"] }) => {
  const [mainImage, setMainImage] = useState(
    URLS.product_thumbnail + "/" + media?.thumbnail || "",
  );
